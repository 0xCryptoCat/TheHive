...
export function Hexagon({
  ...
  const hexStyle: CSSProperties = {
    ...style,
    '--hexagon-color': gradient || color,
    '--hexagon-gradient': gradient,
  } as CSSProperties;

  // Add gradient class if gradient is provided
  const gradientClass = gradient ? 'hexagon-gradient' : '';

  if (animate) {
    return (
      <motion.div
        className={`hexagon ${sizeClass} ${gradientClass} ${className}`}
        style={hexStyle}
